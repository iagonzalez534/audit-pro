...
<style>
    ...
    @media only screen and (max-width: 600px) {
        ...
        .report-content h1 { font-size: 1.8rem !important; }
        .paywall-price { font-size: 3rem !important; }
...
def analyze_business_pro(my_text, comp_text, key):
    ...
    - Cierre: CORTA EL TEXTO a mitad.
    
    ###SEPARADOR###

    PARTE 2 (PREMIUM - 2000 PALABRAS):
...
if not st.session_state.report_pro:
    ...
    if "unlocked" not in st.session_state or not st.session_state.unlocked:
        ...
        st.balloons()
        st.markdown("""<div style="background:#ecfdf5; padding:30px; border-radius:16px; border:1px solid #10b981; margin: 40px 0; text-align:center;"><h2 style="color:#059669; margin:0; font-size:2rem; border:none; background:transparent;">💎 ACCESO VIP CONCEDIDO</h2></div>""", unsafe_allow_html=True)
